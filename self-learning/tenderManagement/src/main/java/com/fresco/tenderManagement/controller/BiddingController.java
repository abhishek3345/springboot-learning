...
@RequestMapping("/bidding")  // all endpoints start with /bidding
public class BiddingController {
    ...
    public ResponseEntity<Object> postBidding(@RequestBody BiddingModel biddingModel) {
        // @RequestBody reads the JSON from request body into BiddingModel
        return biddingService.postBidding(biddingModel);
    }

    @GetMapping("/list")
    public ResponseEntity<Object> getBidding(@RequestParam double bidAmount) {
        // @RequestParam reads ?bidAmount=15000000 from the URL
        return biddingService.getBidding(bidAmount);
    }

    @PatchMapping("/update/{id}")
    public ResponseEntity<Object> updateBidding(@PathVariable int id,
